...
import { motion, AnimatePresence } from 'framer-motion';
import { 
  Truck, Globe, Link, RefreshCw, CheckCircle2, 
  Package, FileCheck, Plus, Search,
  ...
  return (
    <div className="min-h-screen bg-[#05070A] p-4 lg:p-10 text-slate-300 font-sans selection:bg-cyan-500/30">
      <div className="max-w-[1400px] mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 bg-[#0B0F18]/50 p-2 border border-slate-800/50 rounded-[2rem]">
          <nav className="flex gap-1">
            {(['Shipments', 'Zones', 'Couriers'] as const).map((tab) => (
              <button
                key={tab}
                          ...
                          <div className="flex justify-end gap-3">
                            <div className="relative group/btn">
                              <button 
                                ...
                                className={cn(
                                  "p-3 rounded-xl border transition-all",
                                  ship.dispatchProof ? "bg-emerald-500/10 border-emerald-500/40 text-emerald-500" : "bg-slate-950 border-slate-800 text-slate-500 hover:text-cyan-500 hover:border-cyan-500/40"
                                )}
                              >
                                <Package size={18} />
                              </button>
                            ...
                            <div className="relative group/btn">
                              <button 
                                ...
                                className={cn(
                                  "p-3 rounded-xl border transition-all",
                                  ship.deliveryProof ? "bg-emerald-500/10 border-emerald-500/40 text-emerald-500" : "bg-slate-950 border-slate-800 text-slate-500 hover:text-emerald-500 hover:border-emerald-500/40"
                                )}
                              >
                                <FileCheck size={18} />
                              </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <button 
                  onClick={() => setShowAddModal(false)}
                  className="w-full py-5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-2xl text-xs font-black uppercase tracking-[0.3em] transition-all shadow-xl shadow-cyan-500/20"
                >
                  Initialize Shipment
                </button>
              </div>
