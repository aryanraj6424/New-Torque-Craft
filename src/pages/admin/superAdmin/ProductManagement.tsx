...
// --- Reusable Modal (Moved outside to fix focus issue) ---
const ActionModal = ({ title, onClose, children }: any) => (
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
    <motion.div 
      initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2 pt-4 border-t border-slate-800">
                  <input 
                    autoFocus
                    type="text" placeholder="New Category..." 
                    className="flex-1 bg-slate-900 border border-slate-800 p-3 rounded-xl text-sm outline-none focus:border-cyan-500 text-white"
                    value={newCat}
                    onChange={(e) => setNewCat(e.target.value)}
                    onKeyPress={(e) => e.key === 'Enter' && handleAddCategory()}
                  />
